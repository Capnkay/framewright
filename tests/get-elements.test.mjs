...
test('getElements accepts pageName and returns bare array', async () => {
...
test('getElements accepts sectionId and returns bare array', async () => {
  const result = await getElements({ query: { sectionId: '1000000001' }, env: { STORE_TYPE: 'json' } });
  assert.equal(result.status, 200);
  assert.ok(Array.isArray(result.body), 'response must be a bare array');
});

test('getElements accepts fieldIds string and returns bare array', async () => {
  const result = await getElements({ query: { fieldIds: '2000000001,2000000002' }, env: { STORE_TYPE: 'json' } });
  assert.equal(result.status, 200);
  assert.ok(Array.isArray(result.body), 'response must be a bare array');
});

test('getElements accepts fieldIds array and returns bare array', async () => {
