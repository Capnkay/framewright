...
export function DesignTab({ elements, accent, onUpdate, onReorder, onContentChange, scope = "pipeline" }) {
  ...
  const selectedEl = elements.find(e => e.id === selected) || elements[0];
  ...
  return (
    <div className="design-tab" data-testid={scope === "composer" ? "composer-section-editor" : "pipeline-design-tab"}>
      <div className="design-layers" data-testid={tid("design-layers-list")}>
        ...
        {elements.map((el, i) => (
          <div key={el.id} className={`design-layer-row ${selectedEl?.id === el.id ? "selected" : ""}`} onClick={() => setSelected(el.id)} data-testid={tid(`design-layer-${el.id}`)}>
            <span>{ELEMENT_LABELS[el.id] || el.id}</span>
      ...
      <div className="design-canvas-wrap">
        <div className="design-canvas" style={{ width: CANVAS_W, height: CANVAS_H }} data-testid={tid("design-canvas")}>
          {elements.map(el => {
            ...
            return (
              <motion.div
                ...
                onPointerDown={() => setSelected(el.id)}
      ...
      {selectedEl && (
        <div className="design-inspector" data-testid={tid("design-inspector")}>
          <div className="design-inspector-head">{ELEMENT_LABELS[selectedEl.id] || selectedEl.id}</div>
