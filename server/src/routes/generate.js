...
import { emitComponentLLM } from '../generate/emitComponentLLM.js';
...
/**
 * Â§13.1's upload, as it reaches this handler.
 *
 ...
 * `undefined` fails as "no wireframe supplied" â€” a message that sends the reader to
...
/**
 * Â§13.1's accepted formats, enforced here as well as at the perception service.
 * Doing it in both places is not duplication: this one produces Â§13's error envelope
...
/**
 * Â§8's second chokepoint, applied. T-109.
 *
 * WHY IT IS HERE AND NOT ONLY IN sanitiseWrite. sanitiseWrite cleans what the CALLER
 * sent. This runs on what the GENERATOR produced â€” an IR element's `css` comes from a
 * model or a template, travels through the emitter, and reaches the store without ever
 * having been the caller's input. Â§8 makes the store boundary a chokepoint, not the
 ...
 * and killing a generation over a style string would make Â§8 something people route
 * around. What must not happen â€” and now cannot â€” is the value reaching the store.
 */
function safeCss(value, elementName, warnings) {
  if (value === null || value === undefined || value === '') return null;
  if (isSafeCssText(value)) return value;
  warnings.push(
    `${elementName}: the css declaration was rejected by Â§8's allow-list and dropped. ` +
...
function assertValid(result, what) {
  ...
  const detail = (result.errors || [])
    ...
    .join('; ');
  throw new Error(`${what} failed its own schema before insert â€” ${detail}`);
}
...
export async function postGenerate(ctx = {}) {
  ...
  // and body.prompt reached the IR builder unsanitised â€” the Â§8 write-side chokepoint
  ...
  // T-108 opened `wireframe`, T-119 `combined`, T-124 `code`. All four of Â§13's modes
  ...
  // COMBINED IS NOT A THIRD PATH, it is both halves plus Â§6's conflict order. Building
  ...
  // wireframe it is a prompt run. Â§13 requires at least one input and does not require
  ...
  if (body.mode === 'combined' && !usesWireframe && !usesPrompt && !usesCode) {
    return badRequest('mode=combined requires a wireframe image, a prompt or code (Â§13).');
  }
  if (body.mode === 'code' && !String(body.code || '').trim()) {
    return badRequest('mode=code requires a React component in `code` (Â§13).');
  }
  ...
  if (isWireframe) {
    ...
    if (!upload) {
      return badRequest(`mode=${body.mode} requires a wireframe image (Â§13.1).`);
    }
    if (!ACCEPTED_IMAGE_TYPES.has(upload.contentType)) {
      return badRequest(
        `Unsupported image type ${upload.contentType}. Accepted: image/jpeg, image/png, image/webp (Â§13.1).`
      );
    }
    if (upload.size > MAX_IMAGE_BYTES) {
      return { status: 413, body: error('PAYLOAD_TOO_LARGE', `Image is ${upload.size} bytes; the limit is ${MAX_IMAGE_BYTES} (Â§13.1).`) };
    }
  }

  const store = createStore(env);
  const jobStore = createJobStore(env.JOB_STORE_PATH ? { filePath: env.JOB_STORE_PATH } : undefined);
  // Â§11.2's root by default; `ARTIFACT_ROOT` in the env moves it. Job ids
  ...
  // Set by stage 4 when mode=code could not read the pasted component. Â§13's 422.
  ...
  try {
    ...
    // ---- stages 1-3 -------------------------------------------------------
    //
    // ONE CALL TO /perceive, THREE TRACE RECORDS. Â§11.0 numbers normalisation (2) and
    // multimodal-understanding (3) as separate stages, and the perception service runs
    // both behind a single request â€” Â§12 gives it one endpoint, not three. So the call
    // is made once and each record carries the part of the result that belongs to it:
    // stage 2 the normalisation transform Â§6 requires, stage 3 the regions and text.
    ...
    // wireframe to name. A string names why the reference template's slots survived â€”
    // most often "no API key", which Â§16.2 makes a supported state rather than a fault.
    let semanticsReason = null;

    // The image the Â§18 critic loop compares its render against. Stage 2's
    // NORMALISED raster when there is one, because Â§6 puts every IR bbox in
    // normalised space â€” comparing against the raw upload would measure the
    ...
    if (isWireframe) {
      // Stage 1: the upload, persisted as this stage's artifact. Â§11.2 keeps artifacts
      ...
      // than inventing a second stage. Â§11 rule 2 persists the input BEFORE the stage
      // and the output after, which is exactly the shape stage 2 needs: the transform
      // Â§6 requires goes in as `s2-preprocessing-normalization.json`, and the
      ...
      // transform, not pixels, because Â§11.2 makes it a service that never writes
      ...
      await trace.runStage(job.jobId, {
        ...
        run: async () => {
          ...
          if (!s3) {
            // Â§11.1: degraded is "the stage did not do its real work but the pipeline
            // continued" â€” the canonical case being this service unreachable. Throwing
    ...
    const s4 = await trace.runStage(job.jobId, {
      ...
      run: async () => {
        ...
        // T-153 â€” perception's geometry, named. `perceived.ir` describes every wireframe
        ...
        //
        // Wired here rather than "later": AGENTS.md rule 9 â€” a module built and left
        ...
        if (wireframeIr) {
          ...
          if (!named.applied && named.reason) {
            // Not a warning. Â§16.2 calls an unset key "a supported state, not an error",
        ...
        //
        // That is the Â§12 Glass Box criterion, not a nicety. Draining it into
        ...
        // Â§14 and AGENTS.md: the pasted component is PARSED, never executed. codeToIr
        ...
        if (usesCode) {
          try {
            ...
            // downstream would mean substring-matching our own prose â€” which is
        ...
        // Â§6's conflict order lives in resolveConflicts and nowhere else: prompt wins
    ...
    if (s4.status === 'failed' || !s4.output) {
      // A CODE INPUT WE CANNOT READ IS THE CALLER'S 422, NOT OUR 500. Â§13 gives
      ...
      // "that component has no Â§7 element markers in it" and sends them to the
    ...
    const sectionDoc = {
      ...
      jobId: job.jobId,
      prompt: body.prompt || '',
      // Â§2 REQUIRES `variations`, PLURAL, AND A STRING. This handler wrote `variation`
    ...
    // Â§2, at the one moment it matters. sectionValidator.js (T-020) existed and was
    ...
    for (const el of ir.elements) {
      const elementDoc = {
        ...
        // Â§3 REQUIRES ALL THREE and this handler emitted none of them, because
        // elementValidator.js (T-021) was built and never called on the document being
        // written. `loop` is null for everything that is not a Cards element -- Â§3
        ...
        // adding `pageName` failed the Â§9 store-liveness assertion. The cause was not
        // Â§3 and not the hydration path -- it was that the seed inserted hardcoded ids
      ...
      // Â§3, at the one moment it matters -- the same argument as the section above.
    ...
    const s5 = await trace.runStage(job.jobId, {
      ...
      run: async () => {
        ...
        return await emitComponentLLM(ir, body.prompt);
      },
      outputExt: 'jsx'
    });
    ...
    // ---- the Â§18 critic loop, between emission and validation ---------------
    //
    ...
    // this block is a boolean check â€” no browser launched, no bundle built, no
    ...
    if (String(env.CRITIC_LOOP || '').toLowerCase() === 'on' && criticWireframe) {
      criticResult = await runCriticLoop({
        ...
        ir,
        source: s5.output,
        emit: async (nextIr) => await emitComponentLLM(nextIr, body.prompt),
      ...
      // the emitted component renders `data?.[id] || "DEFAULT"` â€” so a critic
      // that fixed a hallucinated heading fixes only the DEFAULT, while the
      // store keeps serving the hallucination. Â§9's hydration then puts the
        ...
        for (const el of currentIr.elements || []) {
          ...
          if (Object.keys(patch).length) {
            try {
              ...
              // Â§18: a gate never fails a generation. A store that would not
    ...
    //
    // USES validateAndRecover's OWN check, which implements Â§18.2's parse-then-lint
    ...
    // callers too.
    //
    // A STRUCTURAL FAILURE IS A DEGRADED STAGE AND A SUCCESSFUL JOB. Â§18.2's whole
    ...
    await trace.runStage(job.jobId, {
      stage: 6,
      // `runStage` invokes `run(input, ctx)` â€” the context is the SECOND argument.
      ...
      // a warning â€” the inverse of what it was written to do. T-152.
      run: async (_input, ctxStage) => {
        ...
        if (!result.ok) {
          ctxStage.addWarning(`Â§18.2 ${result.kind}: ${result.error}`);
        }
    ...
    await trace.runStage(job.jobId, {
      ...
      run: async () => {
        ...
        // emitted source from `job.componentFile`, and nothing was setting it â€” so
    ...
    // Â§11.1's terminal status. `setStatus` existed on the job store and NOTHING
    ...
    // same shape as five other defects this week â€” the API was built, and the call
    // was never made.
    ...
    // Â§12: degradation is part of the contract, not an afterthought. A caller that
    ...
    // T-153 â€” the same honesty one level down. `degraded` says the upload was ignored;
